## =============================================================================
## MODEL3A: FULLY ADJUSTED -- BACKWARD SELECTION using AMI UNADJUSTED 
## (SUPPLEMENTARY TABLES 1 a & b)
## (1) Prep outcome and analysis specific dataset, runs si_fit_model_fullcovariates_bkwdselection.R
##
## Author: Samantha Ip
...
get_vacc_res <- function(event, vac_str, agegp, cohort_vac, agebreaks, agelabels, covars, cuts_weeks_since_expo, master_df_fpath, cohort_start_date, cohort_end_date, noncase_frac=0.1){
  ...
  res_vacc <- fit_model_fullcovariates(covars, vac_str, agebreaks, agelabels, agegp, event, survival_data, cuts_weeks_since_expo, cohort_start_date="2020-12-08", cohort_end_date="2021-03-18", expo="VACCINE", noncase_frac=0.1)
  
  return(res_vacc)
}


ls_agegps_vacall <- lapply(list(agelabels[1], agelabels[2]), #mc.cores=2,
                        function(agelabel)
                          get_vacc_res(event="AMI", vac_str="vac_all", agegp=agelabel, cohort_vac, agebreaks, agelabels, covars, cuts_weeks_since_expo, master_df_fpath, cohort_start_date, cohort_end_date, noncase_frac=0.1))
