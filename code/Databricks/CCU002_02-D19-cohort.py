...
# MAGIC %md %md # CCU002_02-D19-cohort
# MAGIC 
...
for outcome in ["ICVT_pregnancy","other_DVT","DVT_ICVT","DVT_pregnancy","DVT_DVT","fracture","thrombocytopenia","TTP","mesenteric_thrombus","DIC","stroke_isch","other_arterial_embolism","PE","AMI","portal_vein_thrombosis","stroke_SAH_HS"]:
   sql("CREATE OR REPLACE GLOBAL TEMP VIEW " + outcome + " AS SELECT NHS_NUMBER_DEID, record_date AS " + outcome + "_date FROM .ccu002_vac_outcomes_allsources_firstdiag_final_wohes WHERE name = '" + outcome + "'")

# COMMAND ----------

# Generate temporary views for each outcome
for outcome in ["DVT_summ_event","ICVT_summ_event"]:
   sql("CREATE OR REPLACE GLOBAL TEMP VIEW " + outcome + " AS SELECT NHS_NUMBER_DEID, record_date AS " + outcome + "_date FROM .ccu002_vac_dvt_icvt_longformat_sjk WHERE composite_event_type = '" + outcome + "'")

# COMMAND ----------

# Generate temporary views for each outcome
for outcome in ["DVT_summ_event","ICVT_summ_event"]:
   sql("CREATE OR REPLACE GLOBAL TEMP VIEW anydiag_" + outcome + " AS SELECT NHS_NUMBER_DEID, record_date AS anydiag_" + outcome + "_date FROM .ccu002_vac_dvt_icvt_longformat_anydiag_sjk WHERE composite_event_type = '" + outcome + "'")

# COMMAND ----------

# Generate temporary views for each outcome
for outcome in ["THROMBO_plus_ART","Arterial_event","THROMBO_plus_VEN","Venous_event","Haematological_event"]:
   sql("CREATE OR REPLACE GLOBAL TEMP VIEW " + outcome + " AS SELECT NHS_NUMBER_DEID, record_date AS " + outcome + "_date FROM .ccu002_vac_combined_outcomes_longformat WHERE composite_event_type = '" + outcome + "'")
...
for outcome in ["AMI","DIC","DVT_DVT","DVT_ICVT","DVT_pregnancy","ICVT_pregnancy","PE","TTP","fracture","mesenteric_thrombus","other_DVT","other_arterial_embolism","portal_vein_thrombosis","stroke_SAH_HS","stroke_isch","thrombocytopenia"]:
   sql("CREATE OR REPLACE GLOBAL TEMP VIEW anydiag_" + outcome + " AS SELECT NHS_NUMBER_DEID, record_date AS anydiag_" + outcome + "_date FROM .ccu002_vac_outcomes_allsources_anydiag_final_wohes WHERE name = '" + outcome + "'")

# COMMAND ----------

# Generate temporary views for each outcome
for outcome in ["Arterial_event","Venous_event","Haematological_event"]:
   sql("CREATE OR REPLACE GLOBAL TEMP VIEW anydiag_" + outcome + " AS SELECT NHS_NUMBER_DEID, record_date AS anydiag_" + outcome + "_date FROM .ccu002_vac_combined_outcomes_longformat_anydiag WHERE composite_event_type = '" + outcome + "'")
...
for outcome in ["THROMBO_plus_fracture","THROMBO_plus_other_art","THROMBO_plus_DVT_ICVT","THROMBO_plus_DVT","THROMBO_plus_PE","THROMBO_plus_TTP","THROMBO_plus_AMI","THROMBO_plus_other_DVT","THROMBO_plus_stroke_SAH_HS","THROMBO_plus_DIC","THROMBO_plus_portal","THROMBO_plus_MESENT","THROMBO_plus_stroke_isch"]:
  sql("CREATE OR REPLACE GLOBAL TEMP VIEW " + outcome + " AS SELECT NHS_NUMBER_DEID, record_date AS " + outcome + "_date FROM .ccu002_vac_thrombo_outcomes_longformat_individual_events WHERE composite_event_type = '" + outcome + "'")
...
# MAGIC FROM .ccu002_vac_included_patients AS cohort
# MAGIC LEFT JOIN (SELECT * FROM .ccu002_vac_vaccine_status WHERE VACCINE_DOSE = "first dose") AS vaccination ON cohort.NHS_NUMBER_DEID = vaccination.NHS_NUMBER_DEID
# MAGIC LEFT JOIN .ccu002_meds_fix_sjk AS meds on cohort.NHS_NUMBER_DEID = meds.NHS_NUMBER_DEID
# MAGIC LEFT JOIN .ccu002_vac_included_patients_allcovariates_0524 AS covariates on cohort.NHS_NUMBER_DEID = covariates.NHS_NUMBER_DEID
# MAGIC LEFT JOIN .ccu002_vaccine_covariates_BNFchap_corrected AS unique_bnf_chaps on cohort.NHS_NUMBER_DEID = unique_bnf_chaps.ID
