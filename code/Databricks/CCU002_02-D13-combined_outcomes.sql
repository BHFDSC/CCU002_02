...
-- MAGIC gdppr_table = '.ccu002_vac_outcomes_gdppr_final'
-- MAGIC hes_table = '.ccu002_vac_outcomes_hes_final'
-- MAGIC sus_table = '.ccu002_vac_outcomes_sus_final' 
-- MAGIC sus_table2 = '.ccu002_vac_outcomes_sus_final_anydiag' #TWO TABLES FOR SUS 
-- MAGIC deaths_table = '.ccu002_vac_outcomes_deaths_final'
-- MAGIC 
...
-- MAGIC skinny_QA_inclusion_table = '.ccu002_vac_included_patients' 
...
-- MAGIC #Dataset parameters
-- MAGIC #These tables were dervied from the separate notebooks for each of these outcomes
-- MAGIC gdppr_table = '.ccu002_inf_outcomes_gdppr_final'
-- MAGIC hes_table = '.ccu002_inf_outcomes_hes_final'
-- MAGIC sus_table = '.ccu002_inf_outcomes_sus_final'
-- MAGIC deaths_table = '.ccu002_inf_outcomes_deaths_final'
-- MAGIC 
-- MAGIC #Date parameters
...
-- MAGIC 
-- MAGIC #Other data inputs
-- MAGIC skinny_QA_inclusion_table = '.ccu002_inf_included_patients' 
-- MAGIC #skinny patient table that has undergone QA and inclusion/exclusion
-- MAGIC 
-- MAGIC #Final table names
...
-- MAGIC # Source: Workspaces//DATA_CURATION_wrang000_functions
-- MAGIC 
-- MAGIC def create_table(table_name:str, database_name:str='', select_sql_script:str=None) -> None:
...
-- MAGIC   
-- MAGIC def drop_table(table_name:str, database_name:str='', if_exists=True):
-- MAGIC   if if_exists:
...
-- MAGIC --FROM .ccu002_vac_outcomes_sus_final_anydiag
-- MAGIC UNION
-- MAGIC SELECT *
...
-- MAGIC %python
-- MAGIC 
-- MAGIC create_table(table_name=final_table_3, database_name='', select_sql_script=f"SELECT * FROM global_temp.ccu002_vaccine_allsources_outcomes_final_wohes2")
-- MAGIC 
...
-- MAGIC 
-- MAGIC create_table(table_name=final_table_5, database_name='', select_sql_script=f"SELECT * FROM global_temp.ccu002_vaccine_allsources_outcomes_final_wohes2")
...
-- MAGIC 
-- MAGIC 
-- MAGIC CREATE TABLE .ccu002_vac_combined_outcomes_longformat_anydiag AS
-- MAGIC WITH CTE AS (
...
-- MAGIC %python
-- MAGIC create_table(table_name=final_table_4, database_name='', select_sql_script=f"SELECT * FROM global_temp.ccu002_vaccine_allsources_outcomes_combined_order_select")
-- MAGIC 
-- MAGIC #final_table is a parameter name defined at the start so can be used for infection and vaccine
...
--drop table if exists .ccu002_vac_combined_outcomes_firstdiag_thrombo_wohes
...
-- MAGIC     ) as (record_date, composite_event_type, composite_event)
-- MAGIC     FROM .ccu002_vac_combined_outcomes_firstdiag_thrombo_wohes)
-- MAGIC     
-- MAGIC     
-- MAGIC SELECT NHS_NUMBER_DEID, record_date, composite_event_type
...
--from .ccu002_vac_outcomes_allsources_firstdiag_final_wohes
from .ccu002_vac_outcomes_allsources_anydiag_final_wohes
...
-- MAGIC --create table .ccu002_vac_DVT_ICVT_longformat_sjk
-- MAGIC --create table .ccu002_vac_DVT_ICVT_longformat_sjk as
-- MAGIC create table .ccu002_vac_DVT_ICVT_longformat_anydiag_sjk as
-- MAGIC WITH CTE AS (
-- MAGIC     SELECT NHS_NUMBER_DEID,  
-- MAGIC     STACK(2,
...
from .ccu002_vac_DVT_ICVT_longformat_anydiag_sjk
