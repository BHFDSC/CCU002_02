...
-- MAGIC vaccine_data = '.ccu002_vac_vaccine_status_'
-- MAGIC 
...
select * from .ccu002_vac_vaccine_status_
...
-- MAGIC %python
-- MAGIC 
-- MAGIC create_table(table_name=final_table, database_name='', select_sql_script=f"SELECT * FROM global_temp.ccu002_vacc_vaccine_status_final_temp")
-- MAGIC 
-- MAGIC #final_table is a parameter name defined at the start so can be used for infection and vaccine

-- COMMAND ----------

--drop table if exists .ccu002_vac_vaccine_status
...
/*with cte_interval as (SELECT NHS_NUMBER_DEID, VACCINE_DOSE, VACCINATION_DATE, VACCINE_PRODUCT FROM .ccu002_vacc_vaccine_status_final
)
*/
...
from .ccu002_vacc_vaccine_status_final as tbl1 
LEFT JOIN 
     .ccu002_vacc_vaccine_status_final as tbl2
...
--drop table if exists .ccu002_vacc_vaccine_status_final
